...
The POPULAR model is simple based on items popularity, there are no adjustment to be made.
...
It appears that UBCF_pea_k_30 and POPULAR are extremly close to each other.



### Result with Validation dataset
Based on the previous models I have reviewed all being very close, I will Evaluate the validation set (I converted the data earlier to realRatingsMatrix)
...
cat("Final UBCF Evaluation results: \n\n")
...
```{r Final POPULAR Evaluation results, include=FALSE}
...
In Conclusion using the **POPULAR**  algorithm model from the recommenderLab library, keeping **30** items with a predicted movie rating of **4** reports an RMSE of **0.860**.
In this capstone project I learned various ways to improve speed using different classes of data and different libraries that were not introduced in the class.
